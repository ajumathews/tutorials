package com.grace.tutorials.springcloud;
...
public class RoomServicesApplication {
	...
	public Docket api(){
		return new Docket(DocumentationType.SWAGGER_2).groupName("Room").select()
				.apis(RequestHandlerSelectors.basePackage("com.grace.tutorials.springcloud"))
				.paths(any()).build()
				.apiInfo(new ApiInfo("Room Services",
						"A set of services to provide data access to rooms"
						,"1.0.0"
						,null
						, new Contact("Aju Mathews", "https://www.ajumathews.com",null)
						,null
						,null
						, new ArrayList()));
	}
